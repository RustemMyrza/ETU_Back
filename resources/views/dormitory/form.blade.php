...
<div class="tab-content col-md-12" id="custom-tabs-one-tabContent">
    <div class="tab-pane active in ru-content" id="custom-tabs-one-ru" role="tabpanel" aria-labelledby="custom-tabs-one-ru-tab">
        <div class="form-group {{ $errors->has('content') ? 'has-error' : ''}}">
            ...
            <textarea class="ckeditor_textarea" name="content[ru]" id="content[ru]" cols="30" rows="10">{{ isset($translatedContent->ru) ? $translatedContent->ru : ''}}</textarea>
    ...
    <div class="tab-pane fade en-content" id="custom-tabs-one-en" role="tabpanel" aria-labelledby="custom-tabs-one-en-tab">
        <div class="form-group {{ $errors->has('content') ? 'has-error' : ''}}">
            ...
            <textarea class="ckeditor_textarea" name="content[en]" id="content[en]" cols="30" rows="10">{{ isset($translatedContent->en) ? $translatedContent->en : ''}}</textarea>
    ...
    <div class="tab-pane fade kz-content" id="custom-tabs-one-kz" role="tabpanel" aria-labelledby="custom-tabs-one-kz-tab">
        <div class="form-group {{ $errors->has('content') ? 'has-error' : ''}}">
            ...
            <textarea class="ckeditor_textarea" name="content[kz]" id="content[kz]" cols="30" rows="10">{{ isset($translatedContent->kz) ? $translatedContent->kz : ''}}</textarea>
...
<div class="form-group {{ $errors->has('dormitory') ? 'has-error' : ''}}">
    <label for="dormitory" class="control-label">{{ 'Общежитие' }}</label>
    <br>
    <select name="dormitory" id="dormitory">
        <option value="1" {{ isset($dormitory->dormitory_id) ? $dormitory->dormitory_id == 1 ? 'selected' : '' : '' }} >Общежитие 1</option>
        <option value="2" {{ isset($dormitory->dormitory_id) ? $dormitory->dormitory_id == 2 ? 'selected' : '' : '' }} >Общежитие 2</option>
        <option value="3" {{ isset($dormitory->dormitory_id) ? $dormitory->dormitory_id == 3 ? 'selected' : '' : '' }} >Общежитие 3</option>
    </select>
    {!! $errors->first('dormitory"', '<p class="help-block">:message</p>') !!}
</div>
