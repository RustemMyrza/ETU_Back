...
@section('content')
    <div class="card-body">
        ...
        <form method="POST" action="{{ url('/admin/ethicalCodePageDocument/' . $document->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
            {{ method_field('PATCH') }}
            {{ csrf_field() }}

            @include ('ethicalCodePageDocument.form', ['formMode' => 'edit'])
