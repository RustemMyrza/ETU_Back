...
@section('content')
    ...
    <div class="card-body">
        ...
        <a href="{{ url('/admin/news/' . $newsId . '/slider/create') }}" class="btn btn-success btn-sm" title="Добавить новый блок">
                ...
                @if (isset($slider))
                    @foreach($slider as $item)
                            <tr>
                                ...
                                <!-- {{ url("$item->image")}} -->
                                <td>
                                <a href="{{ url('/admin/news/' . $newsId . '/slider/' . $item->id) }}" title="Посмотреть блок"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> Просмотр</button></a>
                                    <a href="{{ url('/admin/news/' . $newsId . '/slider/' . $item->id . '/edit') }}" title="Редактировать блок">
                                        <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-alt"
                                                                                aria-hidden="true"></i> Редактировать
                                        </button>
                                    </a>

                                    <form method="POST" action="{{ url('/admin/news/' . $newsId . '/slider/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                        {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
