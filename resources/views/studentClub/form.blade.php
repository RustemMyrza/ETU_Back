...
<div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="name" type="text" id="name" value="{{ isset($studentClub->name) ? $studentClub->name : ''}}" >
...
<div class="tab-content col-md-12" id="custom-tabs-one-tabContent">
    <div class="tab-pane active in ru-content" id="custom-tabs-one-ru" role="tabpanel" aria-labelledby="custom-tabs-one-ru-tab">
        <div class="form-group {{ $errors->has('description') ? 'has-error' : ''}}">
            ...
            <textarea class="ckeditor_textarea" name="description[ru]" id="description[ru]" cols="30" rows="10">{{ isset($translatedDescription->ru) ? $translatedDescription->ru : ''}}</textarea>
    ...
    <div class="tab-pane fade en-content" id="custom-tabs-one-en" role="tabpanel" aria-labelledby="custom-tabs-one-en-tab">
        <div class="form-group {{ $errors->has('description') ? 'has-error' : ''}}">
            ...
            <textarea class="ckeditor_textarea" name="description[en]" id="description[en]" cols="30" rows="10">{{ isset($translatedDescription->en) ? $translatedDescription->en : ''}}</textarea>
    ...
    <div class="tab-pane fade kz-content" id="custom-tabs-one-kz" role="tabpanel" aria-labelledby="custom-tabs-one-kz-tab">
        <div class="form-group {{ $errors->has('description') ? 'has-error' : ''}}">
            ...
            <textarea class="ckeditor_textarea" name="description[kz]" id="description[kz]" cols="30" rows="10">{{ isset($translatedDescription->kz) ? $translatedDescription->kz : ''}}</textarea>
...
<div class="form-group {{ $errors->has('image') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="image" type="file" id="image" value="{{ isset($studentClub->image) ? $studentClub->image : ''}}" >
    {!! $errors->first('image', '<p class="help-block">:message</p>') !!}
</div>
@if (isset($studentClub->image))
    <div class="form-group">
        <img src="{{ \Config::get('constants.alias.cdn_url').$studentClub->image }}" alt="" width="300px;">
    </div>
@endif
