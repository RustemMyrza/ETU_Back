...
@section('content')
    ...
    <div class="card-body">
        ...
        <a href="{{ url('/admin/bachelorSchool/' . $schoolId . '/page/create') }}" class="btn btn-success btn-sm" title="Добавить новый блок">
        ...
        <form method="GET" action="{{ url('/admin/bachelorSchool/' . $schoolId . '/page') }}" accept-charset="UTF-8"
                    ...
                    @foreach($bachelorSchoolPage as $item)
                            <tr>
                                ...
                                <a href="{{ url('/admin/bachelorSchool/' . $schoolId . '/page/' . $item->id) }}" title="Посмотреть блок"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> Просмотр</button></a>
                                    ...
                                    <form method="POST" action="{{ url('/admin/bachelorSchool/' . $schoolId . '/page' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                        {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
