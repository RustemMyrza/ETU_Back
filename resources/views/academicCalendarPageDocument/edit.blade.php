...
@section('content')
    <div class="card-body">
        ...
        <form method="POST" action="{{ url('/admin/academicCalendarPageDocument/' . $document->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
            {{ method_field('PATCH') }}
            {{ csrf_field() }}

            @include ('academicCalendarPageDocument.form', ['formMode' => 'edit'])
