...
@section('content')
    ...
    <div class="card-body">
        ...
        <a href="{{ url('/admin/ethicalCodePageMeta') }}" class="btn btn-secondary btn-sm" title="Метаданные">
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                @if (isset($translatesData))
                    @foreach($ethicsCodePage as $item)
                        @if($item->getContent != null)
                            <tr>
                                ...
                                <td>
                                <a href="{{ url('/admin/ethicsCodePage/' . $item->id) }}" title="Посмотреть блок"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> Просмотр</button></a>
                                    <a href="{{ url('/admin/ethicsCodePage/' . $item->id . '/edit') }}" title="Редактировать блок">
            ...
            <div class="pagination-wrapper"> {!! $ethicsCodePage->appends(['search' => Request::get('search')])->render() !!} </div>
        </div>
