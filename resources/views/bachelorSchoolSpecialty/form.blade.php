...
<div class="form-group {{ $errors->has('image') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="image" type="file" id="image" value="{{ isset($bachelorSchoolSpecialty->image) ? $bachelorSchoolSpecialty->image : ''}}" >
    {!! $errors->first('image', '<p class="help-block">:message</p>') !!}
</div>
@if (isset($bachelorSchoolSpecialty->image))
    <div class="form-group">
        <img src="{{ url($bachelorSchoolSpecialty->image) }}" alt="{{ url($bachelorSchoolSpecialty->image) }}" width="300px;">
    </div>
@endif
