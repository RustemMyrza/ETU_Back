...
<div class="form-group {{ $errors->has('image') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="document" type="file" id="document" value="{{ isset($mastersSpecialtyDocument->link) ? $mastersSpecialtyDocument->link : ''}}" >
    {!! $errors->first('document', '<p class="help-block">:message</p>') !!}
</div>
@if (isset($mastersSpecialtyDocument->link))
    <div class="form-group">
        <a href="{{ $mastersSpecialtyDocument->link ? url($mastersSpecialtyDocument->link) : '' }}">{{$mastersSpecialtyDocument->name ? Str::limit($translatedName->ru, 50) : ''}}</a>
    </div>
@endif
