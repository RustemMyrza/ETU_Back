...
<div class="form-group {{ $errors->has('block_id') ? 'has-error' : ''}}">
    <label for="block_id" class="control-label">{{ 'Блок' }}</label>
    <br>
    <select name="block_id" id="block_id">
        <option value="1" {{ isset($document->block_id) ? $document->block_id == 1 ? 'selected' : '' : '' }}>Нормативно правовые акты</option>
        <option value="2" {{ isset($document->block_id) ? $document->block_id == 2 ? 'selected' : '' : '' }}>Научные конференции</option>
    </select>
