...
<div class="tab-content col-md-12" id="custom-tabs-one-tabContent">
    <div class="tab-pane active in ru-content" id="custom-tabs-one-ru" role="tabpanel" aria-labelledby="custom-tabs-one-ru-tab">
        <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
            ...
            <input class="form-control" name="name[ru]" type="text" id="name_ru" value="{{ isset($newsDocuments->getName->ru) ? $newsDocuments->getName->ru : ''}}" >
    ...
    <div class="tab-pane fade en-content" id="custom-tabs-one-en" role="tabpanel" aria-labelledby="custom-tabs-one-en-tab">
        <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
            ...
            <input class="form-control" name="name[en]" type="text" id="name_en" value="{{ isset($newsDocuments->getName->en) ? $newsDocuments->getName->en : ''}}" >
    ...
    <div class="tab-pane fade kz-content" id="custom-tabs-one-kz" role="tabpanel" aria-labelledby="custom-tabs-one-kz-tab">
        <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
            ...
            <input class="form-control" name="name[kz]" type="text" id="name_kz" value="{{ isset($newsDocuments->getName->kz) ? $newsDocuments->getName->kz : ''}}" >
...
<div class="form-group {{ $errors->has('document') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="document" type="file" id="document" value="{{ isset($newsDocuments->document_link) ? $newsDocuments->document_link : ''}}" >
    {!! $errors->first('document', '<p class="help-block">:message</p>') !!}
</div>
@if (isset($newsDocuments->document_link))
    <div class="form-group">
        <a href="{{ url($newsDocuments->document_link) }}">
            {{ $newsDocuments->getName->ru ? $newsDocuments->getName->ru : '' }}
        </a>
    </div>    
@endif
