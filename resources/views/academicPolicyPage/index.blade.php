...
@section('title', 'Академическая политика')

@section('content_header')
    <h1>Академическая политика</h1>
@stop
...
@section('content')
    ...
    <div class="card-body">
        ...
        <form method="GET" action="{{ url('/admin/academicPolicyPage') }}" accept-charset="UTF-8"
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                @if (isset($translatesData))
                    @foreach($academicPolicyPage as $item)
                        @if($item->getContent != null)
                            <tr>
                                ...
                                <td>
                                <a href="{{ url('/admin/academicPolicyPage/' . $item->id) }}" title="Посмотреть блок"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> Просмотр</button></a>
                                    <a href="{{ url('/admin/academicPolicyPage/' . $item->id . '/edit') }}" title="Редактировать блок">
                                        <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-alt"
                                                                                aria-hidden="true"></i> Редактировать
                                        </button>
                                    </a>

                                    <form method="POST" action="{{ url('/admin/academicPolicyPage' . '/' . $item->id) }}"
            ...
            <div class="pagination-wrapper"> {!! $academicPolicyPage->appends(['search' => Request::get('search')])->render() !!} </div>
        </div>
